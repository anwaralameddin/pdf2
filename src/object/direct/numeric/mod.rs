...
impl ObjectParser<'_> for Numeric {
    fn parse(buffer: &[Byte], offset: Offset) -> ParseResult<Self> {
        Integer::parse_suppress_recoverable(buffer, offset)
            .or_else(|| Real::parse_suppress_recoverable(buffer, offset))
            .unwrap_or_else(|| {
...
mod tests {
    ...
    #[test]
    fn numeric_invalid() {
        let parse_result = Numeric::parse(b" <", 0);
        let expected_error =
        ...
        let parse_result = Numeric::parse(buffer, 0);
        let expected_error =
